...
Run("command request supports multi group destination beacon payload", TargetBattleCommandRegressionCases.CommandRequestSupportsMultiGroupDestinationBeaconPayload); Run("runtime destination beacon command accepts shared reachable beacon", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconCommandAcceptsSharedReachableBeacon); Run("runtime destination beacon command rejects unreachable multi select atomically", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconCommandRejectsUnreachableMultiSelectAtomically); Run("runtime destination beacon replacement affects only selected groups", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconReplacementAffectsOnlySelectedGroups); Run("runtime destination beacon replacement removes orphaned beacons", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconReplacementRemovesOrphanedBeacons); Run("runtime destination beacon command during pause does not advance time", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconCommandDuringPauseDoesNotAdvanceTime); Run("runtime preparation seeded destination beacon initializes active beacon", TargetBattleCommandRegressionCases.RuntimePreparationSeededDestinationBeaconInitializesActiveBeacon); Run("runtime destination beacon movement uses beacon instead of distant enemy", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconMovementUsesBeaconInsteadOfDistantEnemy); Run("runtime destination beacon flow field builds once for shared profile", TargetBattleCommandRegressionCases.RuntimeDestinationBeaconFlowFieldBuildsOnceForSharedProfile); Run("runtime hold-line command keeps player corps from advancing", TargetBattleCommandRegressionCases.RuntimeHoldLineCommandKeepsPlayerCorpsFromAdvancing);
...
TargetBattleCommandAuthorizationRegressionCases.Register(Run);
TargetBattleRegroupRetreatRegressionCases.Register(Run);
