...
- UI 或服务层返回需要玩家处理的威胁。
- 不应在无确认情况下直接自动结算。
- 如果导航路径不可用，日志出现 `WorldArmyNavigationBlocked`，敌军进入 `NavigationBlocked`，世界推进暂停；不应静默回退为倒计时跳转攻击。
...
- 玩家可选择进入场域探索以揭示战术细节。
- 透明场域不会强制进入探索流程。
- 隐藏入口不会出现在战斗请求的玩家可用入口中；探索记忆揭示入口后才可用。
- 普通直接进入敌方场域只适用于完整战术情报可见的场域；需要探索的敌方场域必须通过抵达部队潜入。
- 探索点行动执行后会写入 `WorldSiteState.Memory`，并影响后续战斗请求的入口、战术标签和探索优势。
- 缺失 `WorldSiteDefinition.Intel` 不应被当作透明情报处理，应显示受限情报并记录诊断日志。
...
- Raid 敌军从墓园出现后会持续沿 `StrategicNavigationTileLayer` 移动，不会停在墓园出口；日志可见 `WorldArmyPathBuilt`。如果出现旧异常攻城状态，只能看到显式阻塞日志，不能看到静默恢复。
- Raid 到达时有明确处理入口。
- 玩家不会在无提示情况下丢失埋骨地。
