...
- `intent_preview`: preview generated from an enemy's currently committed Intent.
...
- Hero movement range.
- Hero push or block target preview.
- Enemy melee and ranged Intent preview.
- Automatic archer target indication.
- AP cost preview for basic actions.

Cards, complex multi-step targeting, and hidden information previews are out of scope for Phase 1.

## Consistency Rules

- The same action must not use different target shapes for preview and resolution.
- Intent display must represent the committed enemy action, not a fresh recalculation every frame.
