...
- Initial strategic navigation surface for moving armies.
...
- Strategic navigation remains an RTS-style pathing implementation detail rather than a grid-tactics gameplay rule.
...
- Empty anchor nodes may keep non-authoritative placeholder map rendering visible, but must not provide army pathing authority.
...
- First-pass strategic navigation let moving armies follow a world-space polyline; the current authoritative Godot navigation contract is documented in `2026-05-05-strategic-godot-navigation-provider.md`.
...
- Empty `ArmySpawnPoints` and `EncounterZones` do not hide non-authoritative placeholder map rendering.
- Strategic army pathing follows the current `StrategicNavigationTileLayer` contract.
