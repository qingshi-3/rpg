...
- Cache each moving army's current path at runtime and only rebuild it when the target or authored navigation data version changes.
...
- If Godot navigation returns repeated no-path results, a start/end point is outside the navigation area, or the navigation layer is missing, the command fails explicitly. A first `godot_navigation_path_empty` during strategic-scene activation is treated as pending navigation and retried.
...
- `WorldArmyState` stores runtime-only cached path points, current path index, destination, authored navigation data version, and transient path-build failure count. These fields are not saved.
...
- Arrival, navigation blocking, garrison transfer, field interception, battle-result resume, and explicit command changes clear cached paths.
...
- If the navigation layer is missing, empty, disconnected, or repeatedly returns no route, army movement enters `NavigationBlocked` and pauses world progression instead of silently choosing another route system.
...
- With missing, empty, disconnected, or persistently failing Godot navigation, movement logs `WorldArmyNavigationBlocked`, marks the army `NavigationBlocked`, pauses world progression, and reports the failure reason.
- A single `godot_navigation_path_empty` immediately after returning to the strategic map logs `WorldArmyNavigationPending`, leaves the army `Moving`, and retries before blocking.
