...
- Product positioning reorganized around three gameplay pillars: Sanguo Qunying-style strategic map, 三国志 / 三国立志传-style officer social management, and turn-based tactical battle.
- Core loop.
- CombatSystem.
- AP ResourceSystem.
- CommandSystem.
...
- Player identity should support the strategic-map, officer-social, and tactical-battle loop; it should not be locked by implementation documents alone.
- Sanguo Qunying is a core gameplay reference for large-map strategy, movement, battle entry, and result writeback; do not reduce it to a technical backend only.
