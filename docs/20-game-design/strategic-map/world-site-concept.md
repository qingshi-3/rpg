...
场域是大世界上的持久运营对象。它可以是营地、矿洞、墓园、道路、桥、遗迹、野外区域或中立据点，但第一版不把它叫“城池”或“城市”，因为它不一定有人口行政结构，也不一定是人工建筑群。
...
战时可承载回合制战斗，并把结果回写到长期状态。
```

因此场域不是三群式薄节点，也不是只有经营按钮的据点 UI。新增场域内容时，应优先判断它是否需要长期状态和可进入空间；如果只是短期遭遇、伏击、救援或夺取，优先建模为野外 `Opportunity` / `EncounterRequest`，不要滥用 `WorldSite`。
