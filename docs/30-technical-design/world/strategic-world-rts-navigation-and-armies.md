...
本文定义战略大地图从“场域节点 + 威胁 marker”升级为“三群式大地图部队移动”的规划。它是 `../../20-game-design/strategic-map/strategic-world-v1.md` 的后续专题，不替代 V1 资源、场域、行动、战斗回写等基础规格。
...
- 如果旧存档或运行中状态已经把玩家小队推进到未配置攻占战的敌方场域，世界层只报错、暂停并提示 `UnsupportedPlayerAssaultArmiesBlocked`；不能静默恢复为空闲小队。
...
- 部队沿连续路径移动。
- 部队移动速度和到达半径可调。
- 无路径时返回清晰失败原因，部队进入 `NavigationBlocked`，世界推进暂停，不静默失败也不自动改道。
