...
- Combat-zone outsider regression: when a commander group is engaged but an individual member footprint is outside the latest combat-zone action bounds, that member paths toward the current combat-zone join region first instead of locking an enemy actor and searching attack slots from outside the zone.
- Combat-zone join bounds regression: when an outsider stands near the selected combat-zone bounds, join movement must path toward the logged action-zone min/max bounds and must not treat the action-zone center as the top-left movement goal.
...
- Split combat-zone regression: if one commander group has members inside more than one global combat zone, members already inside a non-selected combat zone must use that actor-local combat zone for combat decisions instead of falling back to ordinary deployment/objective advance.
...
- 2026-06-05 immediate-frontline regression: `runtime local combat prefers nearest executable attack step over far flank` first failed with a movement-completed corps at `(33,19)` continuing to `(33,20)` toward retained target `bonefield:f6_draugarlord:2` while `bonefield:f6_draugarlord:4` was already in orthogonal attack range, then passed after movement continuation stopped chains whenever the current anchor has an immediate attack opportunity and allowed the next actor decision to attack.
- 2026-06-05 immediate-frontline verification: `dotnet run --project tests\TargetBattleArchitectureRegression\TargetBattleArchitectureRegression.csproj --no-restore` passed, then `dotnet build rpg.csproj -maxcpucount:2 -v:minimal` passed with 0 warnings and 0 errors.
- 2026-06-06 combat-zone outsider regression: `runtime combat-zone outsiders path to latest combat zone before slot search` first failed with `player_rear:1` targeting `enemy_front:1`, then passed after individual actors outside the latest `CombatJoin` action-zone bounds received `combat_zone_join_advance` region movement before target slot search.
- 2026-06-06 combat-zone outsider verification: `dotnet run --project tests\TargetBattleArchitectureRegression\TargetBattleArchitectureRegression.csproj --no-restore` passed, then `dotnet build rpg.csproj -maxcpucount:2 -v:minimal` passed with 0 warnings and 0 errors.
- 2026-06-06 split combat-zone regression: `runtime members inside actor-local combat zone do not march to selected group zone` first failed with `player_right:1` moving to `player_group:combat_join:combat_zone_1`, then passed after actor decisions preferred their own combat-zone membership over the group's selected gather zone when already inside another active combat zone.
