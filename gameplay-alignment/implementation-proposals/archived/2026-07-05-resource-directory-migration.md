...
Status: Accepted - Archived After User QA
...
- Moved LimboAI battle behavior trees from `assets/ai/battle/` into `resource/battle/ai/`.
- Moved LimboAI GDScript task adapters from `scripts/ai/limbo_tasks/battle/` into `src/Runtime/Battle/AI/LimboTasks/`.
- Rewired `BattleAiAgentHost.tscn` and both behavior-tree resources to the new authored-resource and task-adapter paths.
- Updated regression guards so the LimboAI legacy bucket under `assets/` is now expected to be empty.
...
- Static stale-path search found old LimboAI paths only in this active implementation proposal and one archived implementation record; no scene, resource, code, config, or test reference still points at the old executable paths.
- `BattleHitFeedbackRegression` passed, including LimboAI behavior-tree and task-adapter path guards.
- `WorldSiteDeploymentCacheRegression` printed `PASS resource directory taxonomy keeps assets raw and resources authored`, proving the taxonomy guard for this batch, but the full project exited 1 because the current worktree has an unrelated HUD icon expectation in `StrategicWorldResourceBarUsesRollingTicker` while `StrategicWorldHud.tscn` still authors text labels. That UI mismatch is outside Batch 1.
- `dotnet build rpg.csproj -maxcpucount:2 -v:minimal` passed with 0 warnings and 0 errors.
- `git diff --check` passed.
- Godot editor manual open was not run because static references and .NET regression checks covered the Batch 1 path changes.

### 2026-07-05 Batches 2-8

Changes:

- Batch 2 moved battle skill definition resources from `assets/battle/skills/` into `resource/battle/skills/` and rewired the battle skill config and resource-path tests.
- Batch 3 moved UI theme and stylebox resources from `assets/themes/game-ui-skin/` into `resource/ui/themes/game-ui-skin/` and rewired UI scene/test references.
- Batch 4 moved shader resources from `assets/battle/shaders/` and `assets/world/shaders/` into `resource/shaders/battle/` and `resource/shaders/world/`, including `.gdshader.uid` sidecars.
- Batch 5 moved TileSet resources from `assets/tilesets/` into `resource/tilesets/` and rewired map/city/site scene ext_resource paths.
- Batch 6 moved building AtlasTexture icon `.tres` resources into `resource/ui/icons/buildings/foundation/` while keeping source building textures under `assets/textures/world/Buildings/`.
- Batch 7 moved the first-slice indexed unit `unit.tres` and `visual.tres` resources into `resource/battle/units/`, leaving `frames.tres` and raw media under `assets/battle/units/`.
- Batch 8 moved the remaining unit `unit.tres` and `visual.tres` resources into `resource/battle/units/`, updated `BattleUnitFactory` broad discovery to `res://resource/battle/units`, and preserved unit audio definitions under `assets/battle/units/**/audio/audio.tres`.
- Updated the current first-slice hero/corps detail document and the migration cleanup report so active docs no longer name old executable unit-resource paths.
...
- Static stale-path searches for migrated resource families found no executable scene, config, code, resource, test, system-design, or gameplay-design references to old `assets/` locations. Remaining old-path hits are limited to this active migration proposal and archived historical proposal bodies.
- `StrategicManagementRegression`, `WorldSiteDeploymentCacheRegression`, and `BattleHitFeedbackRegression` passed after their corresponding batch changes.
- Unit resource counts after Batch 8: `assets/battle/units/**/unit.tres = 0`, `assets/battle/units/**/visual.tres = 0`, `resource/battle/units/**/unit.tres = 697`, and `resource/battle/units/**/visual.tres = 697`.
- `frames.tres`, raw unit media, PLIST files, and the two unit audio profiles remain under `assets/` by explicit exception.
- `TargetBattleArchitectureRegression` has no remaining migration-path failures, but still exits 1 on the pre-existing oversized-code guard: `BattlePresentation.cs`, `HeroCorps.cs`, `PresentationAntiRot.cs`, and `WorldSiteRoot.SiteManagementHud.cs` exceed their accepted line budgets.

### 2026-07-05 User QA And Archive

Result:

- User ran the post-migration QA pass and reported no issues.
- The resource directory migration is accepted and archived.
- The remaining `TargetBattleArchitectureRegression` oversized-code guard is outside the resource migration scope and should be handled by a separate readability/decomposition workstream.
