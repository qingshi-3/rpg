...
-> destination-beacon flow-field caches when player commands need them
...
Destination-beacon flow-field construction owns command-scoped static direction fields:

- building a distance or direction field from one destination beacon over immutable topology;
- keying caches by beacon id, topology version, height, and footprint/passability profile;
- reusing the field while the beacon and topology/profile inputs are unchanged;
- invalidating the field when the beacon moves, is superseded, expires, or when topology/profile inputs change;
- excluding living-unit occupancy, same-tick reservations, local combat slots, target state, and Presentation facts from the field.
...
- destination-beacon flow-field sampling and region next-step ranking from accepted command or tactical intent;
...
- group-scoped destination beacon fields and coarse route hints such as lanes, gates, chokepoints, portals, or group-owned route anchors;
...
`BattleBeaconFlowField` or an equivalent runtime cache is a command-scoped field derived from immutable topology for one destination beacon and one footprint/passability profile. It is allowed because it is rebuilt on beacon/profile/topology changes, not on every movement boundary. It is not part of authored map data and must not include dynamic blockers, same-tick reservations, combat slots, actor targets, damage, or Presentation state.
...
- Every actor state-machine movement decision boundary validates movement from current actor, active destination beacon or retained target, topology, footprint, occupancy, reservations, and command facts.
- Default assault movement follows the actor's selected battle-group destination beacon or default attack posture. Target acquisition is attack-opportunity first only when the actor has no live retained target, has an immediate attack opportunity, or the active command scope permits reacquisition. A moving actor retains a live target while marching so each movement boundary does not rescore every enemy or rebuild whole-region navigation fields.
...
## Intent-Directed Beacon And Region Movement

Battle-group movement goals are resolved from command, tactical intent, or fallback into target objects, destination beacons, and tactical regions. Player groups normally use runtime destination beacons carried by accepted player commands. Enemy groups use battle-group-owned target objects selected by configurable tactical intent plans. Runtime movement resolution treats all of them as movement goals or route constraints, not as hidden target actors.

Intent-directed beacon or region movement should be resolved before local attack-slot movement:

```text
active destination beacon, selected target object, hold, protect, retreat, or intent-derived region
-> beacon flow field or region candidate anchors
-> route hints from lanes, chokepoints, flank routes, reserve points, or defend points when available
-> footprint-aware next-step ranking
-> local occupancy and reservation validation
```

When no enemy is locked, a battle group advances, holds, or withdraws toward its selected destination beacon, stable target object, or region. When a local target is locked, movement may temporarily switch to attack-slot approach inside the group-owned local combat region. When that target dies, becomes invalid, exceeds pursuit limits, or cannot be reached under the command scope, movement returns to the current destination beacon, AI-intent target object, hold area, protect-hero area, fallback target, or retreat path.
...
For the first beacon-flow implementation, player destination movement is resolved through a beacon flow field plus local neighbor validation. The field gives the preferred static direction toward the beacon for a profile. Local movement still commits only neighboring cells and validates every step against topology, footprint, occupancy, reservations, command facts, and actor state.

Route hints remain low-frequency coarse anchors. They may come from map-authored markers or from a static `BattleRouteTopology` query over chunks, sectors, portals, and clearance profiles. Beacon flow fields are allowed only for command-scoped destination beacons and must be cache-keyed. Runtime must not build route fields for every target, local combat zone, attack slot, or moving actor. Beacon movement, target death, combat-zone membership changes, command target changes, actor capability changes, route profile changes, topology version changes, flow-field invalidation, or route-hint invalidation invalidate the current movement intent and let the state machine choose a new local step at the next valid boundary.
...
This separation is a product rule as much as a performance rule: ordinary movement should read as executing the group beacon, command, or tactical intent, not as every unit independently searching the whole battlefield for the globally best attack anchor.
...
- Runtime hot paths use local neighbor resolution. The state machine selects the active beacon, target, attack slot, support slot, or hold role; the movement resolver ranks nearby anchors against that selected intent and may use a cached beacon flow-field direction as one ranking input.
...
- `StuckRecovery` is a bounded escape rule: if obstacle following does not improve within its configured progress budget, Runtime may switch obstacle side, return to the current beacon flow-field direction or coarse route hint, hold, or emit an explicit movement failure. It must not silently start a whole-map search.
...
- Runtime movement resolution may use an anchor graph, cached placement map, clearance map, or command-scoped destination-beacon flow field. Combat hot paths must not construct new flow fields per actor movement decision; fields are built or reused only when beacon/profile/topology inputs change. Any navigation data must be derived from full-footprint placement legality for the actor size.
...
- respects the actor's destination beacon, held area, defense leash, protect target, or retreat constraint;
...
Authoritative movement intent is not a full precomputed battle path. Cached beacon flow fields, target, slot, region, or local scoring data are advisory only.
...
- destination beacon id, anchor, height, revision, or validity;
- beacon flow-field cache key, version, or passability profile;
- actor anchor;
...
- whether the target, destination beacon, or destination cell exists in topology;
- whether a beacon flow field exists for the required topology/profile key and whether the actor anchor can sample it;
...
- optional objective-zone markers and route-hint markers routed through Application snapshots;
- compiled static route topology derived from battle navigation topology when available;
- destination beacon commands and beacon cache keys from Runtime command state;
...
- command-scoped destination-beacon flow-field caches;
...
- destination-beacon flow fields, when present, are derived from immutable topology and keyed by beacon, topology version, height, and footprint/passability profile;
- beacon flow fields are rebuilt only when beacon/profile/topology inputs change, not every movement decision, fixed tick, or render frame;
- beacon flow fields exclude dynamic occupancy, reservations, local combat slots, actor targets, damage state, and Presentation state;
