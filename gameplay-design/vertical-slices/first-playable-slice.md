...
-> choose a named hero company
-> travel to an enemy strategic location
-> read battle context
-> deploy and plan the assault
...
| Deployed player hero companies per battle | 1 | The player chooses one hero company for the Bonefield assault; the slice does not require multi-company deployment. |
...
### VS-04 Select A Named Hero Company

Player intent: choose a hero-led force, not raw troops.

Player actions:

1. Start an expedition from the stronghold.
2. Select one of the available heroes.
3. Confirm the default corps attached to the selected hero.
...
- The selected hero's default corps is attached automatically for the first slice.
- The player cannot accidentally launch an empty or non-hero expedition in this slice.

Acceptance:

- The selected expedition can be described as "hero + corps".
- The player understands the selected hero's battlefield role before choosing the target.
- The slice offers three selectable hero companies, but only one company is deployed in the Bonefield battle.

### VS-05 Send The Expedition To Bonefield

Player intent: move the hero company through the world toward a hostile objective.

Player actions:

1. Choose Bonefield as the expedition target.
2. Watch the company travel.
3. Arrive at the target.

Required system response:

- The company leaves the stronghold and visibly travels on the strategic map.
...
### VS-07 Deploy The Hero Company

Player intent: place the company before battle and understand the valid deployment area.

Player actions:

1. Enter the deployment screen.
2. Place or confirm the hero company inside a valid deployment zone.
3. Inspect enemy deployment if visible.

Required system response:

- Valid player deployment area is visible.
- Invalid placement is rejected with player-facing feedback.
- The hero and corps remain recognizable after placement.

Acceptance:

- The player cannot start battle with required player units undeployed.
- The deployment step is short enough for one-company play and does not feel like full army setup.
...
1. Select one target or objective region for the hero company.
2. Select one engagement rule.
3. Start battle.
...
- Battle cannot start until the hero company has a valid objective and engagement rule.
...
- deploying multiple player heroes in one battle;
- multiple controllable companies in the same battle;
...
- The player selects one of three named hero companies.
- The expedition travels from stronghold to Bonefield.
- The player deploys and chooses a battle plan.
