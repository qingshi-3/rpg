...
When Runtime starts, the active plan is converted into player-sourced tactical intent and behaves like the default combined-command scope for that battle group. Later accepted hero, corps, or combined commands may supersede part or all of that scope by producing newer player-sourced tactical intent. Supersession must be explicit so runtime events and reports can distinguish:
...
- player-sourced tactical intent or plan-supersession runtime facts;
...
- battle-start plans initialize player-sourced tactical intent instead of a separate player-only movement path;
