...
-> request a battle from strategic state
...
-> player or AI completes deployment and plan choices
...
`BattleStartSnapshot` is the immutable Runtime input. It should contain battle-facing facts such as battle groups, hero and corps definitions, combat stats, location context, objective zones, skill snapshots, and topology references.
...
The long-term handoff payload is a typed bridge/session context, not the legacy `BattleSessionHandoff` request/result store. The scene router may write or cancel this payload during root-scene replacement, but it must not fabricate battle results or strategic consequences.
...
- Missing strategic intent, participant, hero, corps, location, map, deployment zone, objective zone, or navigation context fails the bridge session explicitly.
...
- scene transition carries bridge session context without owning gameplay validation or outcome truth;
