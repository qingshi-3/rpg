...
- setup and cleanup of current migration handoff stores;
...
| `EnterBattlePreparation` | strategic battle trigger | world site battle scene | `BattleSessionHandoff.BeginBattle` during migration |
...
For V0, the router may continue using existing migration handoff stores:

- `StrategicWorldRuntime.BeginSiteVisit`, `TryConsumePendingSiteVisit`, and `MarkWorldResumeAfterSiteReturn`;
- `BattleSessionHandoff.BeginBattle`, `CancelBattle`, and result consumption.

The router owns when those stores are written or canceled for scene-entry transitions. Scene roots may still consume handoff data during migration, but they should not create competing handoff state.

Long-term design may replace these static stores with a unified `SceneTransitionContext`. That replacement is not part of this document's V0 requirement.
...
- Keep existing `StrategicWorldRuntime` and `BattleSessionHandoff` stores.
...
- Consider replacing static handoff stores with a typed `SceneTransitionContext`.
