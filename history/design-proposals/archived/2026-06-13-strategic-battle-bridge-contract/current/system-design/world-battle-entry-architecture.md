...
Deployment preparation consumes side-aware semantic deployment zones, creates or updates site-local placement rows for deployed non-resident forces, applies preferred placements to deployed force requests, records each deployed participating battle group's selected formation, selected objective area, and engagement rule, and exports the current navigation snapshot before runtime starts.

An arriving strategic expedition army may carry multiple hero companies. Battle preparation presents those carried companies as separate deployable player battle groups. The player may leave carried companies undeployed as reserves for the first slice. Reserve companies are retained in the strategic army, but they are excluded from the Runtime battle request, do not spawn, do not need objective or engagement-rule plans, do not suffer battle casualties, and cannot be called in as mid-battle reinforcements until a later accepted proposal adds that flow.
...
Battle-group plan drafting is Application-owned state derived from the current battle request. UI may display and edit the draft through requests, but it must not create a second battle snapshot or unit pool. A draft is launchable only when at least one player-side battle group is deployed and every deployed player-side battle group has valid hero-led formation placement, objective-zone selection, and engagement rule selection. Undeployed reserve groups are not required for launch.
...
- battle preparation can validate deployed battle-group plan completeness before launch while allowing reserve companies to remain undeployed;
