...
Map scenes may define a conventional `SemanticMarkers` root node. Children under that root may include `SemanticMapMarker` nodes.

`SemanticMapMarker` is a Godot editor tool node with these exported authoring fields:
...
Battle preparation should consume `DeploymentZone` markers as an authored constraint on top of the full walkable-surface deployment cache.
