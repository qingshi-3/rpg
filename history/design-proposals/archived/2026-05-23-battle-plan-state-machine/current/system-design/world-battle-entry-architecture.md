...
World battle entry owns conversion from strategic context into `BattleStartRequest`, pre-battle presentation, scene-transition handoff, deployment preparation, and post-battle writeback for the currently implemented world battle kinds.
...
Runtime state includes pending battle request, pending rollback token, pre-battle dialog state, `BattleSessionHandoff`, site deployment cache, battle-preparation placement state, and active battle runtime adapter state.

## Inputs

Inputs are assault-ready player armies, field intercept results, world/site definitions, current site and army state, scene paths, semantic deployment markers, and completed `BattleResult` values.

## Outputs

Outputs are `BattleStartRequest`, `BattleEntranceRequest`, force requests, preferred placement requests, battle-launch diagnostics, `WorldActionResult`, `GameEvent` records, site/army mutations, and world-clock tick advancement after settlement.
...
Deployment preparation consumes side-aware semantic deployment zones, creates or updates site-local placement rows for non-resident forces, applies preferred placements to force requests, and exports the current navigation snapshot before runtime starts.

## Failure Rules

Unsupported assault targets, missing site definitions, empty deployment caches, invalid placement terrain, battle handoff failure, and battle result/request mismatch fail explicitly. Failed battle launch must cancel handoff, disable runtime, clear temporary battle entities, and restore the previous site mode.
