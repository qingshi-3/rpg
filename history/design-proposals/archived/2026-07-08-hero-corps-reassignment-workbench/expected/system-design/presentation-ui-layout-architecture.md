...
While in placement-follow mode, persistent HUD and management controls should move out of the battlefield view or stay compact enough to keep the map readable. The top status bar, compact roster, deployment controls, start-battle button, and nonessential hints may slide offscreen and return after placement commit or cancellation. Deployment-zone highlights, formation preview, and legality feedback stay visible because they are the active placement context.
...
After a battle group is placed, `WorldSitePeacetimeHud.tscn` should show a concise top-center prompt such as `右键选择部队目的地`. This prompt is feedback only: it must use authored HUD nodes, ignore mouse input, and never block map clicks or right-click destination selection.
