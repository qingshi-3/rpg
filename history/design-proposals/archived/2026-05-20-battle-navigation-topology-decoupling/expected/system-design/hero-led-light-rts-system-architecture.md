...
| Infrastructure | Save/load, resource loading, logging, IDs, random seed, time, scene switching, diagnostics, test fixtures. Root scene switching follows `scene-transition-router-architecture.md`. | No business rules. It supports systems without deciding gameplay outcomes. |
...
- Map topology compilation owns the final static battle navigation data. It resolves authored TileMapLayer surfaces, covered lower layers, same-level adjacency, and authored height links into an immutable `BattleNavigationTopology` data contract before Runtime starts.
- Runtime pathfinding consumes only `BattleNavigationTopology` plus actor footprint facts, dynamic occupancy, and same-tick reservations. It must not parse `TileMapLayer`, `BattleGridMap`, `GridCellSurface`, `LayerRole`, water, bridge, or raw height-link authoring concepts.
...
The integration model is driver-first. Behavior trees may decide what an agent wants to do, sequence request/wait/fallback flow, and react to typed result observations. They must not apply movement, damage, settlement, or persistent world changes themselves.
...
`BattleIntentResolver` remains the first-slice action-resolution authority. Runtime migration must preserve `BattleEventStream`, outcome, navigation, footprint, and settlement contracts before replacing hardcoded autonomous combat decisions.

The runtime AI executor migration should expose typed request/result boundaries instead of giving LimboAI direct access to mutable runtime state:
...
The first runtime executor slice may inject a default C# executor into `BattleRuntimeSession` only as a boundary seam. That executor returns typed requests such as hold, advance, wait for charge, or attack. Runtime still selects legal movement cells, consumes attack charge, mutates actor state, emits events, and determines battle outcomes.
...
-> SceneTransitionRouter sets battle handoff and enters the battle scene
