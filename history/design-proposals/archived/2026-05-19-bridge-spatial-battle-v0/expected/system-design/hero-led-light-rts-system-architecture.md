...
Runtime playback may keep independent actor animation lanes for responsiveness, but a damage event must wait for target-lane progress when the target has earlier runtime movement or attack playback that affects the visible contact position. This keeps attacks visually tied to the same actor cells that Runtime emitted without globally serializing unrelated movement.
...
- Map topology compilation owns the final static battle navigation data. It resolves authored surfaces, covered lower layers, same-level adjacency, and explicit height connections into immutable topology data before Runtime starts. Runtime pathfinding consumes only that topology plus actor footprint facts, dynamic occupancy, and same-tick reservations. Battle traversal cost is uniform across walkable topology nodes. Dynamic living-unit occupancy is layered: immediate next-cell occupancy and same-tick reservations are hard blockers, while future projected occupancy is soft route cost.
- Runtime may evaluate several cells ahead, but it commits only one neighbor move per actor action. Each next tick replans from current actor, target, occupancy, reservations, and command facts.
...
- Candidate committed movement is valid only if all cells covered by the next footprint are walkable, unoccupied, and unreserved. Projected future route cells may include occupied cells as extra cost, not immediate legality.
...
### Bridge Spatial Battle V0 Slice

The first spatial battle implementation slice should prove one bridge-style scenario before broadening combat content.

Ownership boundaries:

| Layer | Bridge Slice Responsibility |
|---|---|
| Definitions / Content | Author the bridge map, tactical marker IDs, deployment zones, and scenario text. |
| Application / Services | Extract tactical markers and freeze them into the battle snapshot or battle-context handoff. |
| Runtime / StateMachine | Use square-grid navigation, occupancy, command posture, and actor events to resolve chokepoint contest or collapse. |
| Presentation / UI | Display the authored map, deployment, minimal command/posture controls, and runtime feedback. |
| Report And Settlement | Explain spatial outcomes from runtime events; do not rediscover map markers or recompute battle truth. |

The runtime may use tactical marker facts as hints or attribution labels, but legality stays with the square-grid graph, occupancy, reservations, command validation, and effect rules. A marker must not become a hidden second pathfinding or damage authority.

The LimboAI behavior tree layer may later choose posture or targets for AI actors, but it does not own movement legality, occupancy, damage, battle outcome, settlement, or report truth.
