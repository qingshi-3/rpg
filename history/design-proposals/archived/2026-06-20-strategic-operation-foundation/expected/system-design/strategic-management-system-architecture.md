...
- faction-shared first-version resources and resource production for `Money`, `Food`, `Wood`, and `Ore`;
- city identity, authored construction regions, city building definitions and instances;
- city force capacity, aggregate reserve soldiers, and reserve recovery over world-map time;
...
- resource types;
- city building types, categories, footprints, costs, upgrade chains, construction-region compatibility, and provided capabilities;
...
- settle resource production, reserve recovery, and resource rewards;
...
Buildings enable and support capabilities such as foundation resource income, reserve capacity, common military training, hero recruitment access, workshop support, defense, or later special routes. They do not directly instantiate battle Runtime units or bypass corps muster rules.

The foundation construction model is bounded RTS-style placement: building-panel selection, mouse-attached preview, grid snapping, footprint validation, overlap checks, bounds checks, and construction-region compatibility. It does not include workers, road connectivity, gathering paths, resource distance efficiency, or full RTS production queues.
...
- city building definitions, footprints, basic costs, categories, and construction-region compatibility;
