...
- battle target objects that can be referenced by AI intent, commands, scenarios, diagnostics, and reports;
- AI intent plans that describe what a battle group wants to do to one or more target objects;
...
An AI intent plan is a command-like input for autonomous groups. It describes a verb applied to target selectors plus constraints.
...
- Presentation consumes runtime movement events only. It must not read or execute AI intent.
