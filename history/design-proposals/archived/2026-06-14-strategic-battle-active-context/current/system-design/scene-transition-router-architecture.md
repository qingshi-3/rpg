...
| `EnterBattlePreparation` | strategic battle bridge session | battle preparation/runtime scene | typed bridge session handoff |
...
- Strategic Battle Bridge session context for battle preparation/runtime entry and cancellation.
...
`BattleSessionHandoff` is a legacy migration store, not the long-term battle bridge authority. New Strategic Management battle entry must use the Strategic Battle Bridge session context. A future unified `SceneTransitionContext` is allowed if it preserves this ownership boundary.
...
- Keep only explicitly scoped migration handoff stores while moving player-facing scene changes behind the router. New Strategic Management battle entry must target the Strategic Battle Bridge session context.
