...
The exact bridge contract between Strategic Management and battle is a separate architecture topic. This document only fixes the dependency boundary: Strategic Management may exchange frozen battle input and battle result summaries through a bridge, but it must never read or mutate battle Runtime live state.
...
External battle interaction is through a future battle bridge. The bridge may translate strategic facts into battle input and battle results back into strategic commands, but the bridge must not become a second strategic-rule owner. Exact bridge DTOs and any battle-side changes are out of scope for this document.
...
Owns only strategic-side battle intent and battle-result application. It must not own battle Runtime internals or bridge DTO details.

This subsystem can say "this expedition requests battle against this target" and later "apply this battle result summary", but the detailed bridge contract is owned by a future battle bridge architecture proposal.
...
- Keep existing battle Runtime and battle entry behavior out of this strategic refactor except through the future bridge boundary.
...
- Battle bridge details must be defined separately before implementing battle-side changes.
...
- Missing battle bridge contract blocks battle handoff implementation instead of creating direct Runtime dependencies.
...
- battle integration remains deferred to an explicit bridge contract instead of leaking Runtime state into Strategic Management.
