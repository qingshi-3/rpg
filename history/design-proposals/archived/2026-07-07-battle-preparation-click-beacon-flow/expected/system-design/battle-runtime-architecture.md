...
| Battle-group command execution | Active destination beacon, default attack posture, formation intent, command revision, current battle-group state. |
| Command execution | Current command, accepted runtime order, destination beacon, retreat/protect/follow state, command supersession state. |
| Local movement steering | Current beacon flow-field key, route hint, route corridor id, steering mode, obstacle-follow side, best distance, and bounded stuck-recovery budget for the active movement intent. |
...
| Battle tactical areas | Group-owned action zones, selected destination beacons, selected target objects, target regions, temporary regions, engagement state, perception summary, and replan timing. |
...
| Group route intent | Destination-beacon flow-field references and low-frequency route hints from static route topology, including route profile, next portal or anchor, corridor revision, and invalidation reason. |
...
Observation facts run continuously from authoritative runtime state. They include perception, contact, target object catalog snapshots, destination-beacon reachability facts, global combat-zone candidates, group action-zone snapshots, local slot facts, route-blocking facts, and reachability diagnostics. Observation facts are read-only inputs for decision systems; they do not own command or tactical intent, do not select group destinations by themselves, and do not mutate actor phases.

Battle-group commander state owns command progression and tactical intent for the hero-led group. It decides whether the group is deploying, advancing toward a destination beacon, sensing contact, locked to a local fight, moving actors into attack or support slots, attacking, regrouping, returning to beacon movement, retreating, routed, or defeated. It consumes observation facts and commands, then exposes typed actor intents.

Actor action state machines own only per-actor execution phases such as anchored decision, moving, attack recovery, holding, interrupted, and defeated. They do not decide whether the battle group should pursue a target, remain on a destination beacon, or enter local combat.
...
Target acquisition is not the same as movement continuation. Default assault AI follows the active destination beacon or default attack posture first, then uses local perception to acquire or retain targets inside that scope. Runtime fully scores attack opportunities when acquiring a target, when the retained target is gone or invalid, when an immediate attack opportunity is already in range, or when the active command scope explicitly allows reacquisition. While marching toward a live retained target or beacon, Runtime keeps target ownership sticky and validates only the next movement boundary; it does not rebuild whole-region navigation data for every enemy on every movement step.

Local steering is part of actor movement execution, not a separate commander. A movement intent may carry steering memory so a unit can follow a short static obstacle edge without jittering left and right every boundary. The steering state must reset when command scope, movement intent, destination beacon, retained target, route hint, route corridor, route profile, beacon flow-field key, or topology authority changes. It must also stop when local combat, attack range, skill cast, defeat, or an explicit interrupt takes over the actor phase.

## Battle-Group State Machine

Actor phases remain the low-level action truth. A battle group has its own commander state that expresses command execution across the hero and corps:
...
- accepted deployment, formation, and preparation-seeded destination-beacon facts;
- destination beacon commands and later hero, corps, or combined commands;
- local perception and target validity;
- default attack posture and command-scope policy;
- destination-beacon reachability;
- runtime movement, attack, reservation, and path failure facts.

The default attack posture biases the group to advance toward its active beacon, engage relevant enemies inside command scope, and resume beacon advance when local contact clears. A preparation-seeded beacon is the initial active beacon for a deployed player battle group. Hold, retreat, protect, regroup, and later optional posture commands supersede this default only through accepted commands or explicit fallback rules.

Runtime events must include command state changes when they materially affect movement, target choice, retreat, hold, or battle outcome. Reports should be able to say that a battle group advanced toward a beacon, was delayed by a chokepoint, entered local combat, returned to beacon movement, or retreated.

Command-state logging must stay low-noise. Log meaningful group-state transitions, beacon accepted/moved/rejected/superseded, target lock changes, local-combat entry or exit, regroup, retreat, defeat, and important degradation reasons. Do not log every fixed tick or every movement progress update as a state transition.
...
- beacon, hold, and protect tasks retain budget unless a direct threat or explicit command overrides them;
...
Local-combat movement must not build runtime flow fields on combat hot paths. The battle-group state machine chooses the current intent, beacon, target, attack slot, support slot, queue, hold, or return state. The movement resolver then checks only nearby legal anchors, may sample an existing beacon flow field for non-engaged beacon movement, and picks a step that reduces distance to the selected intent while respecting topology, occupancy, and reservations. If no executable local step exists, the actor degrades to queue, support, hold pressure, or a named failure reason instead of constructing a whole-region field.
...
- Runtime may use destination-beacon flow-field samples, optional objective-zone facts, target-specific attack slots, coarse route hints from static route topology, local steering memory, local reservations, and command facts to choose the next neighbor at each continuation boundary. Beacon flow fields are built or reused when beacon/profile/topology inputs change, not during per-actor movement hot paths. Presentation frames never choose movement targets.
...
Static map-scale route planning is not actor-local. When a player group receives a destination beacon, Runtime should request or reuse the beacon flow field for the required profile. When non-beacon tactical intent needs coarse structure across large barriers such as rivers, long walls, gates, or chokepoints, Runtime may request or reuse a group route hint from immutable route topology. Individual actors then locally advance through Runtime neighbor commits. Dynamic living-unit blockers do not rebuild static route graphs or beacon fields; they degrade to queue, support, hold pressure, retry, or a named failure reason.
...
- accepted `RuntimeOrder`, including destination beacon commands
...
- destination-beacon flow fields are command-scoped advisory facts and cannot authorize movement that fails Runtime topology, footprint, occupancy, or reservation validation;
- changing pathfinding from route hints to beacon flow fields does not change Presentation authority: movement visuals still consume Runtime movement events and interpolation only;
