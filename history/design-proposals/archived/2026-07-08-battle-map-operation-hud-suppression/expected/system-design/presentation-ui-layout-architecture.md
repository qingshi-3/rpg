...
| `BattlePreparation` | player roster, hero/corps deployment, formation, enemy preview summary, start battle | Application battle request/snapshot draft + deployment draft | Deployment draft confirmation only |
...
Allowed visible feedback during HUD suppression is limited to map-owned or non-blocking context: formation previews, deployment-zone highlights, skill range and target cells, destination markers, selected-unit rings, local invalid-placement feedback, cursor-local prompts, and small prompt docks whose controls ignore mouse input. Restoring from suppression must return to the previous battle-preparation or battle-runtime HUD state instead of rebuilding unrelated management UI.
...
Troop option cards must show reserve-soldier and resource consumption directly on the card. When a selected hero already has a corps, the card should also show old-corps refund and final net reserve/resource change. Presentation may display Strategic Management view-model projections for these values, but Strategic Management commands remain the authority for replacement validation and settlement.
...
-> show non-blocking top-center prompt to right-click a destination
-> right-click reachable destination cell to set initial beacon
...
After a battle group is placed, `WorldSitePeacetimeHud.tscn` should show a concise top-center prompt such as `右键选择部队目的地`. This prompt is feedback only: it must use authored HUD nodes, ignore mouse input, and never block map clicks or right-click destination selection. If the player is currently choosing that destination, all larger battle-preparation HUD docks stay suppressed until the right-click target is accepted or the player cancels/returns.

Migration path:

1. Keep existing behavior stable while the panel is moved left.
2. Replace text-heavy battle-preparation panels with a compact roster, battlefield overlays, current-battle-group deployment controls, and fixed start-battle action.
...
5. Reserve `OverlayHost` for deployment highlights, formation-follow previews, invalid-placement feedback, destination beacons, optional route previews, and optional objective/tactical marker outlines.
...
- right-click a valid destination cell in the battle viewport;
