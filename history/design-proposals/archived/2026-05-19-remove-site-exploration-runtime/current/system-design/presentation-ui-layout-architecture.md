...
| `SiteExploration` | exploration status, movement confirmation, alert actions | Site exploration state + Application exploration service | Application/service exploration actions |
...
Exploration HUD can remain separate temporarily, but its controls should eventually bind into the left primary workspace or overlay/modal hosts depending on whether they are persistent status or urgent alerts.
