...
For the first slice, region-directed movement is resolved through local neighbor scoring toward a region anchor or nearest region cell. It must not build runtime route fields for every objective, target, or combat zone. Target movement, target death, combat-zone membership changes, command target changes, or actor capability changes invalidate the current movement intent and let the state machine choose a new local step at the next valid boundary.
...
- Local obstacle avoidance is a bounded neighbor algorithm: choose a legal neighboring anchor that reduces distance, does not collide with occupied or reserved footprints, and does not immediately reverse or oscillate when another improving step exists.
