...
| Local movement steering | Current route hint, steering mode, obstacle-follow side, best distance, and bounded stuck-recovery budget for the active movement intent. |
...
- Runtime may use objective-zone facts, target-specific attack slots, coarse route hints, local steering memory, local reservations, and command facts to choose the next neighbor at each continuation boundary. First-slice Runtime movement uses local neighbor scoring plus stateful local steering rather than runtime flow-field construction. Presentation frames never choose movement targets.
