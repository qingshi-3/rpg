...
- local steering memory for static obstacle following, rejoin, queue/hold, and stuck recovery;
- optional coarse route hints such as objective entrances, lanes, gates, chokepoints, or group-owned route anchors;
...
For the first slice, region-directed movement is resolved through local neighbor scoring and local steering toward a route hint, region anchor, or nearest region cell. Route hints are low-frequency coarse anchors, not a field or full route. They may represent map-authored entrances, lanes, gates, chokepoints, or group-owned approach anchors. Runtime must not build route fields for every objective, target, or combat zone. Target movement, target death, combat-zone membership changes, command target changes, actor capability changes, or route-hint invalidation invalidate the current movement intent and let the state machine choose a new local step at the next valid boundary.
...
- Local obstacle avoidance is a stateful local steering algorithm, not a stateless one-step score. A unit starts in `SeekGoal`; when static topology blocks useful progress it may enter `FollowObstacle`, keep a fixed obstacle side, track the best goal distance reached, and switch back through `RejoinSeek` when normal goal progress becomes available.
...
- `StuckRecovery` is a bounded escape rule: if obstacle following does not improve within its configured progress budget, Runtime may switch obstacle side, return to a coarse hint, hold, or emit an explicit movement failure. It must not silently start a whole-map search.
