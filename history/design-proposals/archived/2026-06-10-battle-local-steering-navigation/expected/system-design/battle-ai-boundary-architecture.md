...
AI may provide or consume coarse route intent such as objective entrance, lane, gate, chokepoint, or group route anchor when those facts are already part of the accepted battle plan or group action zone. AI does not own obstacle following. Runtime movement steering owns the local `SeekGoal`, `FollowObstacle`, `RejoinSeek`, `QueueOrHold`, and `StuckRecovery` mechanics after AI has selected the tactical intent.
...
- Movement blocked by static topology: try Runtime local steering with obstacle following; otherwise stop advance, switch route hint, hold pressure, or emit a failure candidate.
