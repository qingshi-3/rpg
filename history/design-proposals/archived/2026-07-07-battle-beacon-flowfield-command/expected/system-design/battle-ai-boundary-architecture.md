...
Runtime distinguishes three command layers for battle groups:
...
The current execution command is the action currently consumed by the actor or group state machine, such as movement, combat join, attack, support, hold, cast, or return. It may replace command display while a group is engaged, but it is not long-term player intent.

The player command is created only by player input, such as a destination beacon command, or by a later accepted planning mode. It has the highest priority and is cleared only when its destination or objective is completed. Entering combat may cause combat-local execution commands to be shown and consumed, but must not erase the stored player command by itself.

The self-calculated command is a safe fallback. It may be generated only when no player command is active and the group is allowed to act autonomously. It is cleared when combat starts, when it is completed, or when a new player command overrides it.

Player command owns upstream player intent:

- main battle-group target;
- hero, corps, and combined command channel;
- tactical intent input such as attack, defend, protect, retreat, regroup, or hold;
- hero skill cast intent and target;
- whether a new command overrides current autonomous behavior.

Tactical AI owns execution inside active tactical intent:

- player-sourced battle-group intent execution inside the active destination beacon or explicit command scope;
- enemy-sourced intent execution inside the active tactical intent plan and resolved target object;
- ordinary attack target choice;
- without explicit AI override, ordinary assault follows the active destination beacon or default attack posture, senses local enemies inside that scope, then chooses the enemy and footprint-valid attack slot that best fits the command scope;
- combat-zone response: when a relevant global combat zone is active inside command scope, AI may request moving toward that zone, joining that fight, occupying an attack slot, occupying a support slot, queueing, holding support, regrouping, or returning to beacon movement;
...
When a player-commanded group has completed its player command and no new player command exists, safe fallback may choose a self-calculated temporary target region from opposing clusters. This fallback is still below player intent: it never overwrites an active player command and never becomes campaign-persistent truth.

Accepted runtime commands must enter Runtime as player-sourced tactical intent. Deployment validates placement and formation; command UI validates and submits destination beacons and other live commands. Tactical AI and Runtime Tactical Intent consume command input at decision boundaries without turning deployment interaction into a movement system.
...
```text
active tactical intent from player command, destination beacon, future battle-group plan, enemy configuration, or fallback
-> selected destination beacon, target object, or objective region
-> default attack posture or explicit command posture
-> local perception
-> battle-group commander state
-> retained target validation
-> request beacon movement, target pursuit, attack-slot approach, hold, regroup, protect, or retreat
-> Runtime validation
```

The default attack posture allows local perception to interrupt beacon movement for relevant enemies inside command scope, then return to beacon movement when local contact clears. Future posture commands may reintroduce move-first, attack-first, hold, retreat-first, or protect-hero behavior as explicit commands instead of mandatory battle-preparation choices.

Target reacquisition is allowed when the retained target dies, becomes invalid, violates the active scope, or the command posture demands a higher-priority response. It is not allowed merely because another render frame elapsed or because another enemy could be globally scored as slightly better.
...
- A fight is active when recent damage, recent attack intent, close threat, or beacon/route blocking is present.
...
beacon still relevant
-> advance toward beacon
fallback
-> hold or regroup
```

Local combat decisions must preserve player intent precedence. Under the default attack posture, a group may join relevant local fights inside perception and command scope, especially when the fight blocks the beacon route, threatens the actor or battle group, lies near the active corridor, or includes the retained target. Distant incidental fights should not pull a group away from a beacon indefinitely. Future posture commands may narrow or widen this join behavior.
...
Behavior trees output typed intent such as attack, advance to target, advance to beacon, join local combat, hold support, return to beacon movement, retreat, or regroup. Runtime remains responsible for validating whether a requested attack or movement is legal. Movement requests name the beacon, target, region, attack slot, support slot, or hold role; AI does not own flow-field construction.

AI may provide or consume coarse route intent such as lane, gate, chokepoint, portal, or group route anchor when those facts are already part of the accepted command, static route topology, or group action zone. AI may consume beacon reachability and flow-field availability facts, but it does not own route topology construction, flow-field construction, portal search, obstacle following, movement legality, or final pathing truth. Runtime movement steering owns the local `SeekGoal`, `FollowObstacle`, `RejoinSeek`, `QueueOrHold`, and `StuckRecovery` mechanics after AI has selected the tactical intent.

Behavior-tree and C# executor decisions must emit low-noise reason codes for diagnostics, such as `join_recent_damage`, `join_blocks_beacon_route`, `hold_support_attack_slots_full`, `return_beacon_threat_clear`, `reject_outside_leash`, `reject_join_budget_full`, or `reject_no_reachable_slot`.
...
Combat zones are global battlefield facts and are not owned by battle groups. Target objects are reusable tactical references exposed by the battle target object catalog. Group action zones, selected target objects, target regions, temporary target regions, and engagement state are owned by battle groups. Runtime may expose global lookup caches, but those caches are not tactical authorities and must not mutate group intent.

Reusable AI services may build perception facts, combat-zone facts, target object facts, group action-zone facts, target candidates, attack-slot candidates, support-slot candidates, queue/regroup candidates, and degradation reasons. Side policy chooses how to use those services:

- enemy-controlled groups consume enemy-sourced tactical intent plans that select target objects, retarget policies, leashes, fallback intent, and engagement gates;
- scenario defaults may provide enemy intent, but explicit enemy group intent wins over the scenario default;
- enemy hold defense may switch the whole group to active assault after damage, attack, or perception trigger;
- player groups consume player-sourced tactical intent produced by accepted destination beacons or other runtime commands; their beacons, target regions, and posture are controlled by those sources, while a self-calculated temporary region is allowed only when no player command is active.

Non-engaged movement is intent-directed through the group's destination beacon, selected target object, or group action zone. AI should not chase a moving actor or volatile runtime-observed cluster as the ordinary out-of-combat movement target. Actor targets become valid when the group is inside or joining a selected combat zone, when an explicit command targets an actor, or when the active enemy intent explicitly authorizes an actor/cluster pursuit behavior with target stickiness and retarget cooldown.

The Tactical Intent migration now applies to both enemy-controlled and player-commanded groups. Accepted player commands, especially destination beacon commands, remain the highest player intent authority, but Runtime movement should consume their tactical intent through the same selected-target, selected-region, group-action-zone, and local-combat path used by enemies.
...
Outside a selected combat zone, AI requests movement toward the selected target object or group action zone. Inside the selected combat zone, AI may choose the best available local combat target, open attack slot, support slot, queue role, regroup role, or fallback. Outside command scope, AI returns to target-object/group-action-zone movement, command constraints, or safe fallback.
...
Rebuild combat-zone facts after important events such as battle start, first contact, engagement enter/exit, damage, target defeat, movement completion into or out of threat range, command changes, beacon changes, major unit additions, and relevant path or reservation failures. Rebuild group action zones at battle start, command/beacon changes, invalidation, and a fixed tick interval. Movement start may mark a local area dirty without forcing immediate full rebuild.
...
- Movement blocked by map-scale static topology while following a beacon: use the current beacon flow field if valid; otherwise reject or fail the beacon with a reportable reason. For non-beacon tactical intent, request or switch a group-scoped route hint from static route topology when available.
- Movement blocked by short local static topology: try Runtime local steering with obstacle following; otherwise stop advance, keep or invalidate the beacon/route hint according to Runtime diagnostics, hold pressure, or emit a failure candidate.
...
- accepted destination beacon commands and active command state;
...
- typed action requests such as hold, move, attack, join local combat, hold support, return to beacon movement, cast, protect, regroup, or retreat;
...
- Static route topology, route hints, and beacon flow-field facts are advisory AI inputs; they do not let AI or Presentation bypass Runtime movement validation.
