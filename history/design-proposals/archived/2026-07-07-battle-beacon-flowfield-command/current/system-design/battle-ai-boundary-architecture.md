...
The player command is created only by player input or an accepted player battle plan. It has the highest priority and is cleared only when its objective is completed. Entering combat may cause combat-local execution commands to be shown and consumed, but must not erase the stored player command by itself.
...
- player-sourced battle-group intent execution inside the active objective zone and engagement rule;
...
Accepted player battle plans and runtime commands must enter Runtime as player-sourced tactical intent. Deployment and command UI validate and submit the input; Tactical AI and Runtime Tactical Intent consume it at decision boundaries without turning deployment interaction into a movement system.
...
```text
active tactical intent from player command, battle-group plan, enemy configuration, or fallback
-> selected target object or objective region
-> engagement rule
-> local perception
