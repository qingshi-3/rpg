...
- Change Thunder Spiral Break from immediate self-submit into a direction-targeted channel: button press enters target picking, mouse position resolves to a 4-way cardinal direction, preview renders the selected forward 3x3 area, and the second click submits that area center to Runtime.
- Keep Thunder Spiral Break's channeled damage window tied to the submitted direction offset so a fold during the channel moves the ongoing window with the caster without refreshing the channel duration.
...
- Presentation/HUD regression must prove that Thunder Spiral Break does not submit from the skill button, renders the resolved 4-way forward 3x3 area under mouse preview, and submits only after the second click.
- Runtime regression must prove that Thunder Spiral Break damages actors overlapping the submitted directional 3x3 area and does not damage actors behind the caster.

## Current Follow-Up Slice

- 2026-06-20: Finish Thunder Spiral Break Presentation polish only. Use existing authored SpriteFrames FX assets to show a persistent coarse 3x3-area release at the Runtime-submitted target center, while keeping the existing caster hold-frame animation. Do not add precise hand, palm, bone, or socket attachment, and do not change Runtime damage, target selection, channel timing, or fold-continuation rules.
- GodotPrompter skills used for this slice: `using-godot-prompter`, `ability-system`, `csharp-godot`, `godot-testing`, `assets-pipeline`, `scene-organization`, and `godot-code-review`.

## Diagnostics And QA

- Thunder Spiral Break manual QA should confirm that selecting each cardinal direction shows the corresponding 3x3 preview, that a second click is required to submit, that the caster remains locked through the channel, and that the held SpriteFrames cast frame resumes only after Runtime reports the channel duration ending.
- Thunder Spiral Break Presentation QA should confirm that the Runtime `SkillUsed` target center spawns the authored swirl FX over the selected 3x3 damage area for the channel duration, without requiring a precise hand/palm attachment.
...
- 2026-06-20: Archived by user request after the final Thunder Spiral Break Presentation tuning pass. Latest verification evidence remains: `dotnet run --project tests\BattleHitFeedbackRegression\BattleHitFeedbackRegression.csproj -v:minimal`, `dotnet build rpg.csproj -maxcpucount:2 -v:minimal`, and `git diff --check`; `git diff --check` only reported existing CRLF normalization warnings in unrelated test files.
- 2026-06-20: Finished the Thunder Spiral Break Presentation polish slice. `SkillUsed` now routes `first_slice_skill_thunder_spiral_break` through a focused Presentation observer, plays an authored `BattleThunderSpiralFx.tscn` at the Runtime-submitted 3x3 target center for the channel duration, keeps the existing caster hold-frame path, and triggers the existing coarse battle-camera action follow instead of adding hand/palm/socket attachment. The FX core uses the fixed 3x3 logical-tile footprint as its anchor, then applies the accepted 1.5x visual tuning: 16 px tile size times 3 cells times 1.5, or 72x72 px, while suppressing the generic actor-foot cast ring so the release reads as centered on the submitted area rather than on the caster. Verification passed: `dotnet run --project tests\BattleHitFeedbackRegression\BattleHitFeedbackRegression.csproj -v:minimal`, `dotnet build rpg.csproj -maxcpucount:2 -v:minimal`, and `git diff --check`. `git diff --check` reported only existing CRLF normalization warnings in unrelated test files. `WorldSiteDeploymentCacheRegression` and `TargetBattleArchitectureRegression` both passed the thunder-related cases during full runs, then failed on unrelated existing worktree issues: retired building-slot marker cleanup and oversized-file anti-rot budget.
- 2026-06-17: Changed Thunder Spiral Break to direction-targeted 4-way selection. The HUD now enters target picking instead of submitting from the skill button, renders the resolved forward 3x3 preview, and submits the resolved center cell on the second click. Runtime stores the selected channel offset, damages only actors overlapping that directional 3x3 area, and carries the offset through Thunder Mark Fold without refreshing duration. Verification passed: `dotnet run --project tests\WorldSiteDeploymentCacheRegression\WorldSiteDeploymentCacheRegression.csproj -v:minimal`, `dotnet run --project tests\TargetBattleArchitectureRegression\TargetBattleArchitectureRegression.csproj -v:minimal`, `dotnet run --project tests\BattleHitFeedbackRegression\BattleHitFeedbackRegression.csproj -v:minimal`, `dotnet build rpg.csproj -maxcpucount:2 -v:minimal`, `git diff --check`, and `dotnet build-server shutdown`. Manual live battle QA remains pending.
- 2026-06-17: Added SpriteFrames frame-number control for channeled caster presentation. `UnitAnimationComponent` now exposes a SpriteFrames-only hold/resume path that plays an authored cue to a concrete frame, pauses on that frame, and resumes from the next frame after the Runtime action duration; normal cue playback cancels stale holds. `BattleUnitAnimationSet` owns the default channeled skill cast hold frame. Thunder Spiral Break routes `SkillUsed` presentation through this hold path, while Runtime carries the channel duration on the semantic `SkillUsed` event so Presentation does not infer gameplay timing. Verification passed: `dotnet run --project tests\BattleHitFeedbackRegression\BattleHitFeedbackRegression.csproj -v:minimal`, `dotnet run --project tests\TargetBattleArchitectureRegression\TargetBattleArchitectureRegression.csproj -v:minimal`, `dotnet build rpg.csproj -maxcpucount:2 -v:minimal`, `git diff --check`, and `dotnet build-server shutdown`. `git diff --check` reported only existing CRLF normalization warnings in unrelated dirty files.
- 2026-06-17: Tightened the unit body animation presentation backend before adding Thunder Spiral Break frame control. `UnitAnimationComponent` now uses authored `AnimatedSprite2D` / `SpriteFrames` as the only unit body animation backend; the legacy `AnimationPlayer` node/path and procedural body fallback were removed so missing animation resources fail visibly instead of silently switching systems. Skill cast may still reuse the authored SpriteFrames attack cue when `skill_cast` frames are absent, and separate authored skill FX scenes remain allowed. Verification passed: `dotnet run --project tests\BattleHitFeedbackRegression\BattleHitFeedbackRegression.csproj -v:minimal`, `dotnet build rpg.csproj -maxcpucount:2 -v:minimal`, and `dotnet build-server shutdown`.
