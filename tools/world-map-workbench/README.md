...
界面按制作任务拆成五个工作区，不要求一次理解全部功能。顶部只保留保存、撤销、重做等全局操作；左侧选择当前任务后，工具、参数、操作步骤和完成方式会随工作区切换。地图始终保留在中央，图层和对象属性需要时再从右侧打开，底部校验默认只显示摘要。
...
地貌笔刷、擦除和填充直接作用于地图。河流、道路、山脉和多边形工具通过单击增加控制点，双击完成。选择工具可拖动控制点或对象，并在右侧修改稳定 ID 和业务属性。输入框获得焦点时，`V` 和 `Delete` 不会触发全局快捷操作。
