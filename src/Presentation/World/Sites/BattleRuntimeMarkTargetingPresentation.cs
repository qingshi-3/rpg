...
internal static class BattleRuntimeMarkTargetingPresentation
{
        ...
        int landingRadius)
    {
        ...
        int radius = System.Math.Max(1, landingRadius);
        var cells = new List<GridPosition>();
        for (int y = markSurface.Y - radius; y <= markSurface.Y + radius; y++)
        {
            for (int x = markSurface.X - radius; x <= markSurface.X + radius; x++)
            {
