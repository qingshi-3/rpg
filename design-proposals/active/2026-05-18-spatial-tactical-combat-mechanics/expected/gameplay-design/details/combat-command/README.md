...
- Runtime builds the static navigation graph once for the battle handoff, then applies living-unit occupancy and same-tick reservations as dynamic filters.
- Runtime may plan several cells ahead, but it commits only the next neighbor cell. The next tick recalculates from current actor and occupancy facts.
...
Unit body collision is stricter than terrain navigation. Other units cannot occupy or reserve any cell inside the candidate footprint. During a live movement tick, each actor proposes one next anchor cell, reserves the full candidate footprint, and cannot directly swap edges with another same-tick mover. Tick-start occupied cells remain blocked for other actors until the next tick, so a unit cannot step into a large unit's interior or tailgate through a just-vacated footprint during the same resolution step.
