...
Player-facing UI is not an information board. It should expose only the state, choices, and feedback that belong to the player's current context.

Every persistent UI surface should answer:

```text
what is currently selected or happening
why it matters now
what the player can do now
what the cost, risk, or result of that action is
what changed after the action
```

Do not use a broad panel to show every available fact about a city, strategic location, army, or battle state by default. Details, histories, full rosters, facility lists, and diagnostics belong behind focused sub-contexts, compact disclosures, or debug tools unless they are the immediate decision.

When the player enters an action context, such as expedition drafting, site entry, construction, battle entry, or battle preparation, unrelated UI should recede. The active surface should keep target, requirements, cost, risk, confirmation, cancellation, and short feedback visible without making the player scan unrelated lists.

## UI Skin And Texture Resource Rules

Scalable player-facing panels, buttons, slots, and framed controls should be authored as reusable Godot UI resources, not as full images stretched by `TextureRect`.

Use:

- `StyleBoxTexture` for scalable `PanelContainer`, `Button`, popup, slot, and framed-control backgrounds;
- `Theme` or shared `.tres` resources when the same skin applies across several controls;
- `NinePatchRect` only for decorative or scene-level nine-patch backgrounds that are not better expressed as a stylebox;
- `TextureRect` for icons, portraits, fixed decoration, atlas regions, and non-scalable images.

Do not stretch a whole bordered panel or button with `TextureRect`. For atlas textures, cut the intended region first and then use the region as an icon, decoration, or texture-backed style resource.

The first strategic-world skin pass uses `assets/textures/ui/basic-ui/1/` as the main visual source. This keeps the first pass coherent and simple. `assets/textures/ui/basic-ui/2/`, `assets/textures/ui/basic-ui/3/`, and `assets/textures/ui/basic-ui/need-human/` may be evaluated later, but they must not be mixed into the main strategic-world skin by default.

Existing `StyleBoxFlat` resources may remain during migration when outside the focused implementation slice. New or migrated scalable player-facing strategic-world panels and buttons should move to authored texture-backed skin resources unless a focused implementation proposal records a specific exception.
...
Strategic-world selected-location UI must be context-first. The default selected-location panel should not expose every facility, garrison, action, and feedback line as one continuous information board. It should show the selected strategic location's identity, current state, and a small number of current actions. Fuller details may be available inside explicit sub-contexts such as city management, expedition drafting, battle entry, or site reports.

The strategic-world map should remain a meaningful interaction surface. Location markers, hover summaries, action availability, selected targets, active expeditions, and recent outcomes should provide compact feedback near the map or in the reserved notification host instead of forcing the player to read every state change from the left panel.

Strategic-world action flows should temporarily replace unrelated selected-location details with the active decision. For example, an expedition draft should emphasize source, target, selected hero companies, risk/cost summary, confirm, and cancel. Construction or site-entry actions should follow the same rule: the active decision owns the current UI until completed or canceled.
...
- player-facing strategic-world panels serve the current context instead of acting as broad information boards;
- strategic-world action flows replace unrelated details with target, requirement, cost/risk, confirmation, cancellation, and short feedback;
- new or migrated scalable player-facing strategic-world panels and buttons use authored texture-backed skin resources from the accepted first skin source unless a focused implementation proposal records an exception;
