...
Observation facts run continuously from authoritative runtime state. They include perception, contact, target object catalog snapshots, global combat-zone candidates, group action-zone snapshots, local slot facts, route-blocking facts, and reachability diagnostics. Observation facts are read-only inputs for decision systems; they do not own command or AI intent, do not select group objectives by themselves, and do not mutate actor phases.
