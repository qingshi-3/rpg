...
- selected target objects resolved from tactical intent produced by group command, battle plan, enemy configuration, or fallback;
...
- command-source state for the selected target object or region, distinguishing player command, self-calculated fallback, and enemy intent;
...
- tactical intent plans produced by player commands, battle plans, enemy configuration, or scenario defaults;
...
- accepted player battle plans and runtime commands may provide player-sourced intent;
...
Player policy is separate at the source boundary, not at the movement algorithm boundary. Player groups reuse perception facts, combat-zone building, local target/slot solving, target object catalogs, selected-region storage, group action zones, and Runtime validation. Player target regions and posture are changed only by player-sourced tactical intent from accepted player commands or accepted battle plans. Enemy intent must never rewrite player group intent.

Player-commanded groups may still enter a player-scoped engaged state from perception, damage, attack, combat-zone overlap, or route-blocking facts when the active tactical intent or engagement rule allows local response. That transition authorizes combat-zone participation and slot solving inside the current player command scope; it does not convert the group to enemy offense, replace the selected objective, or override player-sourced intent.
...
The current execution command is the state-machine action being consumed now, such as region advance, combat join, attack, support, hold, cast, or return. The player command is highest priority and is cleared only when its objective is completed. The self-calculated command exists only while there is no player command; it is cleared on combat entry, objective completion, or player-command override.
...
- player-commanded groups consume accepted player battle plans through the same tactical intent and region movement path as enemy groups;
