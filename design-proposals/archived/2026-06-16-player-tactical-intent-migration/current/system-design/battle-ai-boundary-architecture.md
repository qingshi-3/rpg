...
- enemy group intent execution inside the active tactical intent plan and resolved target object;
...
- enemy-controlled groups consume tactical intent plans that select target objects, retarget policies, leashes, fallback intent, and engagement gates;
...
- player groups may reuse the same tactical solvers and may enter player-scoped engagement; their player-command target regions and posture are controlled by player commands or accepted battle plans, while a self-calculated temporary region is allowed only when no player command is active.
...
The first Tactical Intent migration applies to enemy-controlled groups only. Player-commanded movement stays under accepted player commands and battle plans until a later accepted migration changes that scope.
