...
The anchor cell is always the footprint's top-left cell. Static terrain legality evaluates the full committed candidate footprint, not only the anchor. Every covered cell in that committed footprint must be walkable on the battle graph for movement or placement to be legal.

Other units cannot occupy or reserve any cell inside the committed candidate footprint. During a live movement tick, each actor proposes one next anchor cell, reserves the full candidate footprint, and cannot directly swap edges with another same-tick mover. Tick-start occupied cells remain blocked for the immediate step until the next tick, so a unit cannot step into a large unit's interior or tailgate through a just-vacated footprint during the same resolution step. Future projected occupancy remains soft route cost, not hard terrain legality, because those units may move before the actor reaches that part of the route.
...
The first spatial implementation slice should be one bridge assault or bridge defense scenario. It is a validation slice, not the full command system.

The player-facing goal is to prove that map shape matters before expanding content breadth:

- a narrow bridge, gate, or pass creates a real chokepoint through square-grid navigation and occupancy;
- the player can at least choose a simple posture such as hold, advance, retreat, or regroup through the accepted command boundary;
- ranged pressure or protected positions matter because they interact with the chokepoint;
- battle events can explain whether the frontline held, collapsed, overextended, or retreated too late.

The slice should use authored tactical markers where they add meaning, but the marker alone is not the rule. The bridge must affect runtime through normal map, snapshot, navigation, command, and event contracts.

Do not expand this slice into a full hero skill set, multiple battle maps, a complete reserve system, or a new AI architecture. Those are later slices after the bridge scenario proves the spatial loop.
