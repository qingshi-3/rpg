...
- Runtime may use objective-zone facts, target-specific attack slots, local reservations, and command facts to choose the next neighbor at each continuation boundary. First-slice Runtime movement uses local neighbor scoring rather than runtime flow-field construction. Presentation frames never choose movement targets.
