...
When Runtime starts, deployed player groups enter the default attack posture without requiring objective-zone or engagement-rule choices. Later accepted hero, corps, or combined commands, including destination beacon commands, produce player-sourced tactical intent. Supersession must be explicit so runtime events and reports can distinguish:
...
A destination beacon command is the default live battle movement command for player battle groups.
...
Presentation may create local hover previews and invalid-input feedback, but it must not commit a beacon by itself. Application validation checks ownership, selected deployed battle groups, tactical-pause allowance, and basic destination availability. Runtime acceptance revalidates battle-context facts, topology version, footprint/passability profiles for every selected group, and whether a beacon flow field can reach the destination.
...
Commands issued during tactical pause may place or update beacons and command facts, but they do not advance movement, attacks, cooldowns, perception, flow-field consumption, or battle time until Runtime resumes.
...
- accepted deployment and formation facts from battle preparation;
- `CommandRequest`;
...
- deployment plans can initialize placement and formation without becoming UI-owned combat truth;
