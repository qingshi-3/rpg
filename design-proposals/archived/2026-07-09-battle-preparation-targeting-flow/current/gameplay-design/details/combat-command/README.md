...
5. After placement, show a non-blocking top-center prompt telling the player to right-click a destination for the selected group.
6. Right-click a reachable destination cell during battle preparation to create or move the selected battle group destination beacon.
7. Repeat the flow for every participating battle group, or multi-select deployed groups and assign them one shared destination beacon.
8. Confirm deployed groups with initial destination beacons and start battle.
...
During placement-follow mode, persistent UI should avoid covering the battlefield so the player can read the map. The roster, top status, deployment controls, and start-battle button may slide offscreen or stay compact, then return after placement is committed or cancelled. Deployment legality feedback must remain visible through the formation preview and deployment-area highlight. The top-center destination prompt must not block map clicks or right-click destination selection.

The default battle-group posture is attack. The player expresses movement intent through destination beacons in both preparation and live battle:
