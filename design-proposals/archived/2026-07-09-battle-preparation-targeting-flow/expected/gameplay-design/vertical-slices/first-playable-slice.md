...
3. After placement, choose an initial destination for the placed battle group with the focused map-targeting interaction.
4. Leave any non-deployed carried battle groups in reserve.
5. Inspect enemy deployment if visible.

Required system response:

- Valid player deployment area is visible.
- Invalid placement is rejected with player-facing feedback.
- After placement, a curved arrow guide from the placed battle group to the pointer makes the required destination choice visible while normal HUD remains hidden.
...
- The player cannot start battle until at least one carried battle group is deployed and required initial destination facts are valid.
