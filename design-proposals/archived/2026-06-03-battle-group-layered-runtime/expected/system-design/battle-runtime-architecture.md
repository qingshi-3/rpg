...
Observation facts run continuously from authoritative runtime state. They include perception, contact, local-combat candidates, local slot facts, route-blocking facts, and reachability diagnostics. Observation facts are read-only inputs for decision systems; they do not own command intent, do not select regions by themselves, and do not mutate actor phases.
...
- Runtime may use objective-zone route fields, target-specific attack slots, local reservations, and command facts to choose the next neighbor at each continuation boundary. Presentation frames never choose movement targets.
