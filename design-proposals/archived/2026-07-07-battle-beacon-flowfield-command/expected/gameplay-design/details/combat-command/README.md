...
Battle preparation should remain a short deployment step. It creates valid starting placement and formation facts for each participating battle group, but it no longer requires objective-region or engagement-rule selection before the battle clock starts.
...
5. Repeat the flow for every participating battle group.
6. Confirm deployed groups and start battle.

The preparation roster is a compact index and drag source, not a long information panel. It should show only battle-group identity and deployment status, such as deployed, reserve, or invalid. Detailed deployment facts belong to the selected battle-group feedback, map overlays, or launch validation feedback.

During drag placement, persistent UI should move away from the battlefield so the player can read the map. The roster, top status, deployment controls, and start-battle button may slide offscreen while dragging, then return after release. Deployment legality feedback must remain visible through the formation preview and deployment-area highlight.

After battle starts, the default battle-group posture is attack. The player expresses movement intent through destination beacons:

1. Select one or more battle groups during live battle or tactical pause.
2. Right-click a valid destination cell.
...
Destination beacons are not pure UI pings. They are runtime command target objects with an owner command, location, version, validity state, and reportable command source. Moving a beacon or issuing a new beacon for the same selected groups supersedes their previous player destination at the next valid command boundary.
