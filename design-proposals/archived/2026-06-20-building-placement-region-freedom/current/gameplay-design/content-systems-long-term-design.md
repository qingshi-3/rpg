...
Player groups may reuse target objects, perception, local combat-region, target, slot, and Runtime validation systems, but the first intent-architecture implementation does not migrate player movement. Player target regions and posture remain controlled by player commands or accepted player battle plans. Enemy intent must not rewrite player intent.
...
| ConstructionRegions | Authored buildable areas that constrain city building placement and category legality. |
...
The player chooses a building from a construction panel, sees a mouse-attached preview, and places it onto a legal snapped grid position inside a compatible construction region. First-foundation legality checks cover footprint, overlap, bounds, region compatibility, and simple eligibility. The first foundation does not include workers, road connectivity, gathering range, resource pathing, or distance-based production efficiency.
